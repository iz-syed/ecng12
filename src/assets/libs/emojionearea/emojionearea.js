...
; (function (factory, global) {
    ...
    function getTemplate(template, unicode, shortname, unicodeChar = "") {
        ...
        if (shortname) {
            friendlyName = shortname.substr(1, shortname.length - 2).replace(/_/g, ' ').replace(/\w\S*/g, function (txt) {
                if (shortname.indexOf(':flag_') != -1 && txt.indexOf('flag') == -1) {
                    //alert(shortname);
                    return get_country_name(txt.toLowerCase());
                } else {
        ...
        var unicodeCharacter = (!unicodeChar || emojione.convert(unicode) == unicodeChar) ? emojione.convert(unicode) : unicodeChar;
        return template
            .replace('{name}', shortname || '')
            .replace('{friendlyName}', friendlyName)
            .replace('{img}', imagePath + (emojioneSupportMode < 2 ? fname.toUpperCase() : fname) + '.' + imageType)
            .replace('{uni}', unicode)
            .replace('{alt}', unicodeCharacter);
    };
    ...
    function unicodeTo(str, template) {
        return str.replace(uniRegexp, function (unicodeChar) {
            ...
            if (typeof unicodeChar !== 'undefined' && unicodeChar in map) {
                return getTemplate(template, map[unicodeChar], "", unicodeChar);
            }
            return unicodeChar;
        });
    }
    ...
    function textFromHtml(str, self, postData = {}) {

        let fbContentReplacement = postData.fbContentReplacement || [];
        let liAnnotations = (postData.callsToAction &&
            postData.callsToAction.liAnnotations &&
            postData.callsToAction.liAnnotations.annotations.length) ? postData.callsToAction.liAnnotations.annotations : [];
        let fbMentionsByKey = {};
        let lnMentionsByKey = {};
        fbContentReplacement.map((item) => {
            fbMentionsByKey[item.value] = item.key;
        });
        liAnnotations.map((item) => {
            lnMentionsByKey[item.entity] = item.name;
        });
        ...
        var sanitizeNode = function (node) {
            if (node.nodeType === TEXT_NODE) {
                ...
                else if (node.className === 'mention') {

                    var span_val =  node.innerText || node.textContent;
                    if (span_val) {
                        line.push('@'+span_val);
                    }
                    return;
                }
                else if (node.className === 'fbmention') {

                    var span_val = node.getAttribute('data-screen-name') || '';
                    span_val = (fbMentionsByKey[span_val]) ? fbMentionsByKey[span_val] : ""
                    if (span_val) {
                        line.push(span_val);
                    }
                    return;
                }
                else if (node.className === 'lnmention') {

                    var span_val = node.getAttribute('data-screen-name') || '';
                    span_val = (lnMentionsByKey[span_val]) ? lnMentionsByKey[span_val] : ""
                    if (span_val) {
                        line.push(span_val);
                    }
                    return;
        ...
        var children = self.editor[0].childNodes;
        for (var i = 0; i < children.length; i++) {
            sanitizeNode(children[i]);
        }
        ...
        str = lines.join('\n');//(browser.is_firefox )?lines.join(''):lines.join('\n');
    ...
    function updateRecent(self, show) {
        ...
        if (!self.recent || self.recent !== emojis || show) {
            if (emojis.length) {
                ...
                self.recentCategory.children(".emojibtn").on("click", function (e) {
                    e.stopPropagation();
    ...
    function init(self, source, options) {
        ...
        self.source.off("saveSelection").on("saveSelection", function (e, val) {
            var ss = saveSelection(editor[0]);
            if (ss && ss.startContainer) {
                if (ss.startContainer === editor[0] || $(ss.startContainer).parents('.emojionearea-editor').length) {
                    if (app.is(".focused") || (val && val.force_save)) self.selection = ss;
                    // if ( app.is(".focused") ) {
                    // window.arun = { startOffset: ss.startOffset, startContainer: ss.startContainer, endOffset: ss.endOffset, endContainer: ss.endContainer };
                    self.last_selcted_pos = { startOffset: ss.startOffset, startContainer: ss.startContainer, endOffset: ss.endOffset, endContainer: ss.endContainer };
                    console.log('self.last_selcted_pos', self.last_selcted_pos);
                    //}   
                }
            }
        });

        self.source.off("valueupdate").on("valueupdate", function (e, val) {
            ...
            var pasteText = function (text) {
                ...
                var caret = $("#" + caretID);
                if (caret.length) {
                    var top = caret.offset().top - editor.offset().top,
        ...
        attach(self, editor, ["mousedown", "mouseup", "click", "keyup", "keydown", "keypress", "input"]);
        ...
        self.on("@filter.click", function (filter) {
            ...
            .on("@emojibtn.click", function (emojibtn) {
                ...
                if (self.standalone) {
                    ...
                    // if (self.last_selcted_pos) restoreSelection(editor[0], self.last_selcted_pos);
                    // else if (self.selection) restoreSelection(editor[0], self.selection);
            ...
            .on("@source.change", function () {
                //self.setText(source[sourceValFunc](), true);
                // trigger('change');
            })
        ...
        self.trigger("loaded", editor);
        //}, self.id === 1); // calcElapsedTime()
        window.unicodeToImage = function (str) {
            str = emojione.shortnameToUnicode(str);
            return unicodeTo(str, self.emojiTemplate);
        };
    ...
    var cdn = {
        defaultBase: "https://s3.amazonaws.com/my-media.eclincher/emoji/",
        defaultBase3: "https://s3.amazonaws.com/my-media.eclincher/emoji/",
        base: null,
        isLoading: false
    };
        ...
        if (!cdn.isLoading) {
            if (!emojione || getSupportMode(detectVersion(emojione)) < 2) {
                ...
                if (getSupportMode(emojioneVersion) > 5) {
                    ...
                    emojioneJsCdnUrlBase = cdn.defaultBase3 + emojioneVersion;
                } else {
                ...
                if (window.emojione) {
                    ...
                    var sprite;
                    if (emojioneSupportMode > 4) {
                        cdn.base = cdn.defaultBase3 + "" + emojioneVersion;
                    ...
                    $.ajax({
                        url: 'https://s3.amazonaws.com/my-media.eclincher/emoji/emoji.js',//emojioneJsCdnUrlBase + "/lib/js/emojione.min.js",
                        dataType: "script",
                        cache: true,
                        success: function () {
                            emojione = window.emojione;
                            ...
                            sprite = 'https://s3.amazonaws.com/my-media.eclincher/emoji/emoji.css';
                            if (options.sprite) {
                ...
                if (emojioneSupportMode > 4) {
                    ...
                    cdn.base = cdn.defaultBase + emojioneVersion;
                }
            }

            cdn.base = 'https://s3.amazonaws.com/my-media.eclincher/emoji/3.1'
    ...
    EmojioneArea.prototype.setText = function (str, is_html) {
        var self = this;
        emojioneReady(function () {
            if (!is_html)
    ...
    EmojioneArea.prototype.getText = function (postData) {
        return textFromHtml(this.editor.html(), this, postData);
    }
...
function unicode_to_image(input) {

    if (!input) {
        return '';
    }

    if (window.unicodeToImage) {
        return window.unicodeToImage(input);
    }
    else if (window.custom_ParseEmoji) {
        return window.custom_ParseEmoji(input);
    } else {
        return input;
    }

    // return (window.unicodeToImage) ? window.unicodeToImage(input) : input;
}


function placeEditorCaretAtEnd(el) {
    ...
    if (typeof window.getSelection != "undefined"
        ...
        var last_child_node = el.childNodes[el.childNodes.length - 1];

        if (last_child_node != undefined && last_child_node.tagName != undefined && last_child_node.tagName.toLowerCase() == 'br') {
            range.setEnd(el, (el.childNodes.length - 1));
        }
...
function cust_restoreSelection(range) {
    if (range) {
        if (window.getSelection) {
            sel = window.getSelection();
            sel.removeAllRanges();
...
function move_to_selected_node(node, el, offset) {
...
function customUnicodeToShort(str) {
    if (!window.emojione) return str;
    str = window.emojione.toShort(str);
    return str;
}
function custom_ParseEmoji(str) {

    if (!window.emojione) return str;

    str = window.emojione.shortnameToUnicode(str);

    uniRegexp = window.emojione.regUnicode;
    window.emojione.imageType = "png";
    var emojioneVersion = "3.1";
    var emojiSize = window.emojione.emojiSize + "/";
    var cdn = {
        defaultBase: "https://s3.amazonaws.com/my-media.eclincher/emoji/",
        defaultBase3: "https://s3.amazonaws.com/my-media.eclincher/emoji/",
        base: null,
    ...
    window.emojione.imagePathPNG = cdn.base + "/png/" + emojiSize;
    window.emojione.imagePathSVG = cdn.base + "/svg/" + emojiSize;
    window.emojione.imagePathSVGSprites = cdn.base + "/sprites/emojione.sprites.svg";

    var template = '<img alt="{alt}" class="emojioneemoji" src="{img}"/>';

    return str.replace(uniRegexp, function (unicodeChar) {
        var map = window.emojione['jsEscapeMap'];
        if (typeof unicodeChar !== 'undefined' && unicodeChar in map) {
            return customEmojiTemplate(template, map[unicodeChar]);
        }
        return unicodeChar;
    });
}

function customEmojiTemplate(template, unicode, shortname) {

    var imageType = window.emojione.imageType, imagePath;
    if (imageType == 'svg') {
        imagePath = window.emojione.imagePathSVG;
    } else {
        imagePath = window.emojione.imagePathPNG;
    }
    var friendlyName = '';
    if (shortname) {
        friendlyName = shortname.substr(1, shortname.length - 2).replace(/_/g, ' ').replace(/\w\S*/g, function (txt) {
...
function get_country_name($key) {
    var country_list = {
        "af": "Afghanistan",
        "ax": "Aland Islands", "al": "Albania", "dz": "Algeria", "as": "American Samoa",
        ...
        "ba": "Bosnia and Herzegovina", "bw": "Botswana", "bv": "Bouvet Island (Bouvetoya)",
        "br": "Brazil", "io": "British Indian Ocean Territory (Chagos Archipelago)",
        ...
        "cc": "Cocos (Keeling) Islands", "co": "Colombia", "km": "Comoros the",
        "cd": "Congo", "cg": "Congo the", "ck": "Cook Islands", "cr": "Costa Rica",
        ...
        "fk": "Falkland Islands (Malvinas)", "fj": "Fiji the Fiji Islands", "fi": "Finland",
        "fr": "France", "gf": "French Guiana", "pf": "French Polynesia",
        "tf": "French Southern Territories", "ga": "Gabon", "gm": "Gambia the",
        ...
        "je": "Jersey", "jo": "Jordan", "kz": "Kazakhstan", "ke": "Kenya", "ki": "Kiribati", "kr": "Korea",
        "kw": "Kuwait", "kg": "Kyrgyz Republic", "la": "Lao", "lv": "Latvia", "lb": "Lebanon", "ls": "Lesotho",
        ...
        "na": "Namibia", "nr": "Nauru", "np": "Nepal", "an": "Netherlands Antilles", "nl": "Netherlands the",
        ...
        "pe": "Peru", "ph": "Philippines", "pn": "Pitcairn Islands", "pl": "Poland", "pt": "Portugal, Portuguese Republic",
        ...
        "rs": "Serbia", "sc": "Seychelles", "sl": "Sierra Leone", "sg": "Singapore", "sk": "Slovakia (Slovak Republic)",
        "si": "Slovenia", "sb": "Solomon Islands", "so": "Somalia, Somali Republic", "za": "South Africa",
        "gs": "South Georgia and the South Sandwich Islands", "es": "Spain", "lk": "Sri Lanka", "sd": "Sudan",
        "sr": "Suriname", "sj": "Svalbard & Jan Mayen Islands", "sz": "Swaziland", "se": "Sweden",
        "ch": "Switzerland, Swiss Confederation", "sy": "Syrian Arab Republic", "tw": "Taiwan", "tj": "Tajikistan",
        ...
        "vi": "United States Virgin Islands", "uy": "Uruguay, Eastern Republic of",
        "uz": "Uzbekistan", "vu": "Vanuatu", "ve": "Venezuela", "vn": "Vietnam", "wf": "Wallis and Futuna",
        "eh": "Western Sahara", "ye": "Yemen", "zm": "Zambia", "zw": "Zimbabwe", "ss": "South Sudan", "bq": "Bonaire", "sx": "Sint Maarten"
    };

    if (country_list[$key] != undefined) {
